...
public class FilePicker extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(b != null)
            activity = b.getInt("key");

        filenames = FetchFileList();

        // specify an adapter (see also next example)
        mAdapter = new FileListAdapter(activity, filenames, this);
    ...
    private ArrayList<String> FetchFileList() {
        ...
        for (int i = 0; i < files.length; i++)
        {
            ...
            if (file_name.endsWith("final_result.mp4") && (activity==1 || activity==4))
                filenames.add(file_name.split(".mp4")[0]);
            if (file_name.endsWith("after_Processing.csv") && activity==2)
                filenames.add(file_name.split(".csv")[0]);
            if (file_name.endsWith("count_persons.csv") && activity==3)
                filenames.add(file_name.split(".csv")[0]);
        }
